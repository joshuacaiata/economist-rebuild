...
class BankPolicy(nn.Module):
    def __init__(self, config, input_size, output_size):
        super(BankPolicy, self).__init__()
        
        bank_config = config.get("bank_agent_training", {})
        
        fc_hidden_sizes = bank_config.get("fc_hidden_sizes", [128, 128])
        
        self.lstm_hidden_size = bank_config.get("lstm_hidden_size", 128)
        self.lstm_num_layers = bank_config.get("lstm_num_layers", 1)
        
        post_lstm_hidden_sizes = bank_config.get("post_lstm_hidden_sizes", [128, 64])
    ...
    def forward(self, x, lstm_state=None):
        ...
        logits = self.policy_head(post_lstm_features)
